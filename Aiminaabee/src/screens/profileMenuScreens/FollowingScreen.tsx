...
import { FOLLOWING } from '../../requests/users';
...
export const FollowingScreen: NavigationFunctionComponent = ({ componentId, userId }: any) => {
  //services
  ...
  const { loading, data, networkStatus, refetch, fetchMore } = useQuery(FOLLOWING, {
  ...
  if (data && data.following) {
    dataProviderWithData.current = dataProviderWithData.current.cloneWithRows(data.following);
  }
    ...
    if (networkStatus !== NetworkStatus.fetchMore && dataProvider.getSize()) {
      fetchMore({
        ...
        updateQuery: (pqr, { fetchMoreResult }) => {
          ...
          return {
            following: [...pqr.following, ...fetchMoreResult.following],
          };
        },
      });
    }
  ...
  return (
    <View flex-1 bg-bgColor>
      {dataProviderWithData.current.getSize() ? (
        ...
        <EmptyScreen title={'you are not following anyone'} />
      )}
    </View>
  );
};

FollowingScreen.options = {
  bottomTabs: {
    visible: false,
    drawBehind: true,
  },
